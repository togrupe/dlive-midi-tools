...
| Mixing Station does not respond | Confirm the REST API is enabled in Mixing Station (Global Settings → API: HTTP REST) and that the port (default: 8080) matches the tool. Verify the host IP is correct and the device running Mixing Station is on the same network. |
